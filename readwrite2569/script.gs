...
const SPREADSHEET_ID = "1f4yYr-p022JLhjPfPt-TkDl83JRT4nWlueH9xe7lTSY"; 
...
function doPost(e) {
  try {
    ...
    else if (action === 'syncRoster') {
      return handleSyncRoster(request);
    }
    else if (action === 'getSchoolReport') {
      return getSchoolReport();
    }
...
// อ่านเกณฑ์ (Criteria) เป็น Map: { criteriaId: [{min,max,label}, ...] }
function buildCriteriaMap(ss) {
...
// readOnly = true : ไม่เขียน header และไม่สร้างชีตห้อง (อ่านจาก Data ตรงๆ ถ้ายังไม่มีชีต) — ใช้กับรายงาน/อ่านล้วน
function loadRoomDataInternal(ss, targetRoom, targetLevel, readOnly) {
  ...
  const criteriaMap = buildCriteriaMap(ss);
  ...
  if (!roomSheet) {
    ...
    if (roomStudents.length === 0) {
      return { config: config, criteria: criteriaMap, students: [] };
    }

    // โหมดอ่านล้วน: ไม่สร้างชีต แค่ map นักเรียนจาก Data (คะแนนยังว่างทั้งหมด)
    if (readOnly) {
      const students = roomStudents.map(r => {
        const s = { rowIndex: null, id: r[0], title: r[1], name: r[2], surname: r[3], room: r[4], scores: {} };
        config.forEach(c => { s.scores[c.colIndex] = (r[c.colIndex - 1] === "" ? null : r[c.colIndex - 1]); });
        return s;
      });
      return { config: config, criteria: criteriaMap, students: students };
    }

    // โหมดปกติ: สร้างชีตห้องจากต้นฉบับ
  ...
  // Update headers (ข้ามในโหมดอ่านล้วน เพื่อไม่ให้มีการเขียนชีต)
  if (!readOnly) {
    config.forEach(c => {
...
// *** SCHOOL REPORT: อ่านข้อมูลทุกห้องในคำขอเดียว (อ่านล้วน ไม่เขียนชีต) สำหรับ Export PDF ทั้งโรงเรียน ***
function getSchoolReport() {
  ...
  const report = rooms.map(roomObj => {
    try {
      const data = loadRoomDataInternal(ss, roomObj.room, roomObj.level, true); // readOnly
      return { info: roomObj, config: data.config, criteria: data.criteria, students: data.students };
    } catch (e) {
      // ห้องที่ตั้งค่าไม่ครบ/ผิดพลาด ไม่ทำให้รายงานทั้งหมดล่ม
      return { info: roomObj, config: [], criteria: {}, students: [], error: e.toString() };
    }
  });

  return responseJSON({ status: 'success', data: report });
}

// *** SYNC ROSTER: อัปเดตรายชื่อห้องให้ตรงกับต้นฉบับ (Data) โดยไม่แตะคะแนนเดิม ***
function handleSyncRoster(payload) {
  const lock = LockService.getScriptLock();
  try {
    lock.waitLock(15000);
  } catch (e) {
  ...
  try {
    const ss = getSS();
    const targetRoom = payload.room;
    const targetLevel = payload.level;
    const removeMissing = payload.removeMissing === true;

    if (!targetRoom) return responseJSON({ status: 'error', message: 'ไม่ระบุห้องเรียน' });

    const masterSheet = ss.getSheetByName(SHEET_DATA_MASTER);
    if (!masterSheet) return responseJSON({ status: 'error', message: 'ไม่พบชีตต้นฉบับ (Data)' });

    const allData = masterSheet.getDataRange().getDisplayValues();
    const header = allData[0];
    const masterRows = allData.slice(1).filter(r => r[4] == targetRoom);
    const masterIds = new Set(masterRows.map(r => String(r[0])));
    ...
    if (!roomSheet) {
      // ยังไม่เคยสร้างชีตห้อง → สร้างใหม่ทั้งห้องจากต้นฉบับ
      if (masterRows.length === 0) {
        return responseJSON({ status: 'error', message: `ไม่พบนักเรียนของห้อง ${targetRoom} ในชีตต้นฉบับ (Data)` });
      }
      roomSheet = ss.insertSheet(targetRoom);
      roomSheet.appendRow(header);
      roomSheet.getRange(2, 1, masterRows.length, masterRows[0].length).setValues(masterRows);
      added = masterRows.length;

    } else {
      const roomData = roomSheet.getDataRange().getDisplayValues();
      const existingIds = new Set(roomData.slice(1).map(r => String(r[0])));

      // 1) เพิ่มนักเรียนใหม่: มีในต้นฉบับ แต่ยังไม่มีในห้อง (append ต่อท้าย คะแนนว่าง)
      const toAdd = masterRows.filter(r => !existingIds.has(String(r[0])));
      if (toAdd.length > 0) {
        const startRow = roomSheet.getLastRow() + 1;
        roomSheet.getRange(startRow, 1, toAdd.length, toAdd[0].length).setValues(toAdd);
        added = toAdd.length;
      }

      // 2) ลบนักเรียนที่ไม่มีในต้นฉบับแล้ว (ทำเฉพาะเมื่อสั่ง removeMissing)
      if (removeMissing) {
        const rowsToDelete = [];
        for (let i = 1; i < roomData.length; i++) { // ข้าม header
          if (!masterIds.has(String(roomData[i][0]))) {
            rowsToDelete.push(i + 1); // แปลงเป็นเลขแถวจริงในชีต (1-based)
          }
        }
        // ลบจากล่างขึ้นบน เพื่อไม่ให้ index เลื่อน (แถวที่เพิ่งเพิ่มอยู่ด้านล่าง ไม่ได้รับผลกระทบ)
        rowsToDelete.sort((a, b) => b - a).forEach(rowNum => roomSheet.deleteRow(rowNum));
        removed = rowsToDelete.length;
      }
    }

    // โหลดข้อมูลห้องล่าสุดกลับไปให้ frontend เรนเดอร์ใหม่ (คะแนนเดิมไม่ถูกแตะต้อง)
    const roomData = loadRoomDataInternal(ss, targetRoom, targetLevel);
    return responseJSON({
      status: 'success',
      added: added,
      removed: removed,
      message: `อัปเดตรายชื่อเรียบร้อย (เพิ่ม ${added} คน${removeMissing ? ', ลบ ' + removed + ' คน' : ''})`,
      ...roomData
    });
